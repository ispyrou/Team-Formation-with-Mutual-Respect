...
def compute_ranking_score(solution_dict,w_weights):
	...
	for category in included_list:
		...
		for i in range(len(nba_dataset_ranking_sorted_by_name)):
			if category not in weights:
				weights[category] = {}
			weights[category][nba_dataset_ranking_sorted_by_name[i][0]] = len(nba_dataset_ranking_sorted_by_name) - nba_dataset_ranking_sorted_by_name[i][1]
...
#				if weights[category][nba_dataset_ranking_sorted_by_name[i]] < weights[category][nba_dataset_ranking_sorted_by_name[j]]:
					A[i][j] = weights[category][nba_dataset_ranking_sorted_by_name[i][0]] - weights[category][nba_dataset_ranking_sorted_by_name[j][0]]
					A[j][i] = weights[category][nba_dataset_ranking_sorted_by_name[j][0]] - weights[category][nba_dataset_ranking_sorted_by_name[i][0]]
#				elif weights[category][nba_dataset_ranking_sorted_by_name[i]] > weights[category][nba_dataset_ranking_sorted_by_name[j]]:
#					A[i][j] = weights[category][nba_dataset_ranking_sorted_by_name[j]] - weights[category][nba_dataset_ranking_sorted_by_name[i]]
#					A[j][i] = weights[category][nba_dataset_ranking_sorted_by_name[i]] - weights[category][nba_dataset_ranking_sorted_by_name[j]]
		Adjacency_m[category] = A
	return weights, Adjacency_m
...
def iqp_rank_algorithm(M, B, C, nba_dataset_ranking, included_list, skill_author_index_dict, w_weights):
...
#	prob.solve(solver=cp.MOSEK, verbose=True)
	prob.solve(solver=cp.GUROBI, verbose=True, TimeLimit=100)
	...
	for i in range(len(included_list)):
		for j in range(len(skill_sol[i])):
			if skill_sol[i][j] == 1:
				solution_dict[included_list[i]] = nodes[j][0]

	score = compute_ranking_score(solution_dict, w_weights)
...
#	weighted = True
	...
	for year in range(2010,2018,1):
		...
		# creating weights dictionairy and adjacency matrices
		w_weights, A = create_adjacency_matrices(nba_dataset_ranking, included_list)

		# creating matrix M
		M = create_matrix_m(A, included_list)
		print("created M")
		
		# creating constraint matrices
		B, C = create_constraint_matrices(A)
		
		# solving
		loop_start_time = time.time()
		iqp_rank_solution, iqp_rank_score = iqp_rank_algorithm(M, B, C, nba_dataset_ranking, included_list, skill_author_index_dict, w_weights)
...
#		break
		...
		if os.path.isfile('results_antisymmetric_nba_6h.pkl'):
			with open('results_antisymmetric_nba_6h.pkl', 'rb') as results:
				results_dict = pickle.load(results)
			with open('results_antisymmetric_nba_6h.pkl', 'wb') as results:
