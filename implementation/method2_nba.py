...
def create_matrix_v(Adjacency_m, current_categories_list):
	num_of_nodes = Adjacency_m[current_categories_list[0]].shape[0]
	num_of_skills = len(current_categories_list)
	V = np.zeros((num_of_nodes*num_of_skills, num_of_nodes*num_of_skills))
	for m in range(num_of_skills):     
		for n in range(num_of_skills):
			if m != n:
				for i in range(num_of_nodes):
					for j in range(num_of_nodes):
						if i != j:
							adj_m = Adjacency_m[current_categories_list[m]]
							adj_n = Adjacency_m[current_categories_list[n]]
							if adj_m[j][i] == 1:
								V[(m*num_of_nodes)+i][(n*num_of_nodes)+j] += 1
							if adj_n[i][j] == 1:
								V[(m*num_of_nodes)+i][(n*num_of_nodes)+j] += 1
#							for category, adj in Adjacency_m.items():
#								if category == current_categories_list[m]:
#									if adj[j][i] == 1:
#										V[(m*num_of_nodes)+i][(n*num_of_nodes)+j] += 1
#								else:
#									if adj[i][j] == 1:
...
def create_constraint_matrices(Adjacency_m):
	...
	D = np.ones(num_of_categories*num_of_nodes)
	return B, C, D

def iqp_rank_algorithm(V, B, C, D, nba_dataset_ranking, included_list, skill_author_index_dict):
	x = cp.Variable(len(C)*len(B), boolean=True)
	
	objective = cp.Minimize(cp.quad_form(x, V))

	constraints = [np.all((B @ x) == 1), np.all((C @ x) <= 1), (D @ x) == len(included_list)]
...
def main():
	...
	for year in range(2010,2018,1):
		...
		'''short_nba_dataset_ranking = nba_dataset_ranking
								for category, ranking in short_nba_dataset_ranking.items():
									ranking.sort(key=lambda elem: elem[0], reverse=True)
									short_nba_dataset_ranking[category] = ranking[:25]
									short_nba_dataset_ranking[category].sort(key=lambda elem: elem[1], reverse=True)
								skill_author_index_dict = create_skill_author_index(short_nba_dataset_ranking)'''


		max_score_solution = max_score_algo(nba_dataset_ranking,weighted)
#		max_score_solution = max_score_algo(short_nba_dataset_ranking,weighted)
...
#			max_score = compute_ranking_score(max_score_solution,short_nba_dataset_ranking,skill_author_index_dict)
		...
		Adjacency_m = create_adjacency_matrices(nba_dataset_ranking, included_list)
#		Adjacency_m = create_adjacency_matrices(short_nba_dataset_ranking, included_list)
		
		# creating matrix M
		V = create_matrix_v(Adjacency_m, included_list)
		
		# creating constraint matrices
		B, C, D = create_constraint_matrices(Adjacency_m)
		
		# solving
		loop_start_time = time.time()
		iqp_rank_solution, iqp_rank_score = iqp_rank_algorithm(V, B, C, D, nba_dataset_ranking, included_list, skill_author_index_dict)
#		iqp_rank_solution, iqp_rank_score = iqp_rank_algorithm(M, B, C, short_nba_dataset_ranking, included_list, skill_author_index_dict)
		...
		if os.path.isfile('results_method2_nba.pkl'):
			with open('results_method2_nba.pkl', 'rb') as results:
				results_dict = pickle.load(results)
			with open('results_method2_nba.pkl', 'wb') as results:
