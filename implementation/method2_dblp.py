...
#			if author_to != author_from and graph.has_edge(author_from,author_to):
			if author_to != author_from and graph.has_edge(author_to, author_from):
				print("has edge")
#				score += graph[author_from][author_to]['weight']
				score += graph[author_to][author_from]['weight']
#				existing_edges.append((author_from,author_to))
				existing_edges.append((author_to,author_from))
			if author_to != author_from and graph.has_edge(author_from, author_to):
				print("has reverse edge")
...
def iqp_graph_algorithm(V, B, C, D, category_graphs, current_categories_list):
	x = cp.Variable(len(C)*len(B), boolean=True)
	print("Length of x is:", len(C)*len(B))
#	V = 0.5 * (V * np.transpose(V))
	
	objective = cp.Minimize(cp.quad_form(x, V))

	constraints = [np.all((B @ x) == 1), np.all((C @ x) <= 1), (D @ x) == len(current_categories_list)]
...
def main():
	...
	for customized_categories in customized_categories_list:
		...
		for category, graph in category_graph_clean.items():
			...
			category_edge_list = list(graph.edges())
			reverse = nx.DiGraph()
			reverse.add_nodes_from(category_node_list)
			for edge in category_edge_list:
				reverse.add_edge(edge[1], edge[0])
				reverse[edge[1]][edge[0]]['weight'] = 1
			reverse_category_graph_clean[category] = reverse
					
		# creating adjacency matrices
		Adjacency_m = create_adjacency_matrices(reverse_category_graph_clean, current_categories_list)
		
		# creating matrix M
		V = create_matrix_v(Adjacency_m, current_categories_list)
#		print(V.shape)
		...
		iqp_graph_solution, iqp_graph_score, iqp_graph_edges = iqp_graph_algorithm(V, B, C, D, reverse_category_graph_clean, current_categories_list)
		...
		dct = {'solution': iqp_graph_solution, 'score': iqp_graph_weighted_score, 'time': loop_elapsed_time_iqp_graph, 'edges': iqp_graph_edges}
		if os.path.isfile('results_method2_dblp.pkl'):
			with open('results_method2_dblp.pkl', 'rb') as results:
				results_dict = pickle.load(results)
			with open('results_method2_dblp.pkl', 'wb') as results:
